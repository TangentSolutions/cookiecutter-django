...
from common.templatetags.table_helpers import table_is_filtered
...
class TestTableIsFiltered:
    ...
    def test_table_is_filtered_no_filters(
        self, request_factory: RequestFactory, table: Table
    ):

        request = request_factory.get("fake-url")
        assert table_is_filtered(table, request) is False

    def test_table_is_filtered_with_filters(
        self, request_factory: RequestFactory, table: Table
    ):
